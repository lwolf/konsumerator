...
import (
	...
	"github.com/lwolf/konsumerator/api/v2"
)
...
func TestNewPrometheusMP(t *testing.T) {
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			_, err := NewPrometheusMP(tlog.NullLogger{}, &v2.PrometheusAutoscalerSpec{
				Address: tc.addrs,
			}, "test")
			if err != nil {
				if len(tc.expErr) == 0 {
...
func TestPrometheusMP_Update(t *testing.T) {
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			...
			pm, err := NewPrometheusMP(tlog.NullLogger{}, &v2.PrometheusAutoscalerSpec{
				Address: addrs,
				Production: v2.ProductionQuerySpec{
					Query:          fmt.Sprintf("%s/%d", tc.queryType, tc.expectedProductionRate),
					PartitionLabel: "partition",
				},
				Consumption: v2.ConsumptionQuerySpec{
					Query:          fmt.Sprintf("%s/%d", tc.queryType, tc.expectedConsumptionRate),
					PartitionLabel: "partition",
				},
				Offset: v2.OffsetQuerySpec{
