...
kind-load-image: ## Load new docker image into KIND cluster
	kind load docker-image --name "konsumerator" ${IMG}

.PHONY: kind-update
kind-update: docker-build kind-load-image deploy ## Build and update image inside KIND cluster

.PHONY: kind-apply
kind-apply: ## Deploy test application into KIND cluster
